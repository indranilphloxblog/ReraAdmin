...
export class IntercepterService {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if(request.url.includes(this.config.IFIX_URL)){
      if(request.url.includes('getDynamicMenuTicketDtls_new')){
        request = request.clone({
          setHeaders: {
            'Authorization': `${this.common.getCGMToken()}`,
          },
        });
      }
    }else {
    ...
    return next.handle(request).pipe(map((event1: HttpEvent<any>): any => {
      ...
      if (event1 instanceof HttpResponse) {
        if( event.url?.indexOf(this.config.IFIX_URL) === -1) {
          // try{
