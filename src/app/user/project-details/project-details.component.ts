...
export class ProjectDetailsComponent implements OnInit, OnChanges {
    ...
    this.rest.getFormInfo(data).subscribe((res: any) => {
      ...
      if (res.success) {
        ...
        if (res.amt) {
          ...
          this.totalAmount = Number(Number(res.amt).toFixed(2));
        }
  ...
  previewDetails() {
    ...
    this.common.signatureImg = this.signatureImg;
    this.common.redirectURL = this.router.url;
    this.router.navigate(['/user/project-preview'])
}
