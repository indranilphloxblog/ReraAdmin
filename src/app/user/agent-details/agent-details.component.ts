...
export class AgentDetailsComponent implements OnInit, OnChanges {
  ...
  getPaymentDetails() {
    ...
    this.rest.getPaymentDetails(data).subscribe((res: any) => {
      if (res.success) {
      if(res.response) {
  ...
  getFormInfo() {
    ...
    this.rest.getProfileFormInfo(data).subscribe((res: any) => {
      ...
      if (res.success) {
        ...
        setTimeout(() => {
          this.hideFieldsBasedOnMatchStr('_OTP');
          this.hideFieldsBasedOnMatchStr('_otp');
        }, 1000);
      }
    }, (err: any) => {
  ...
  getFormSubmitInfo() {
    ...
    this.rest.getFormSubmitInfo(data).subscribe((res: any) => {
        ...
        if (res.success) {
            this.signatureImg = '';
            if (res.response[0].signature != null && res.response[0].signature != undefined && res.response[0].signature != '') {
                this.signatureImg = this.rest.FILE_ROOT + res.response[0].signature;
            }
        }
    });
}
  ...
  hideFieldsBasedOnMatchStr(str: string) {
    try{
      const elems: any = document.querySelectorAll('[id*="'+str+'"]');
      console.log(elems);
  ...
  previewDetails() {
    this.common.stepListShow = this.stepListShow;
    this.common.formDetails = this.formDetails;
    this.common.paymentdetails = this.paymentDetails;
    this.router.navigate(['/user/agent-preview-details']);
    let path = window.location.href.split('pages');
    sessionStorage.setItem('prevurl',path[1])
}
