...
export class WorkflowComponent implements OnInit {
  @ViewChild('editNote') private editNote: any;
  promotername: string = '';
  projectname: string = '';
  stepworkid: number = 0;
  ...
  displaynotequerysteps = [this.CEO_ROLE_SEQ, this.SCRUTINY2_ROLE_SEQ, this.HOR_ROLE_SEQ, this.SECRETARY_ROLE_SEQ, this.CHAIRMAN_ROLE_SEQ, this.MEMBER_ROLE_SEQ];
  displaynotesteps = [this.CEO_ROLE_SEQ, this.INTERN_ROLE_SEQ, this.HOR_ROLE_SEQ, this.SECRETARY_ROLE_SEQ, this.CHAIRMAN_ROLE_SEQ, this.MEMBER_ROLE_SEQ];
  displayceosteps = [this.CEO_ROLE_SEQ, this.HOR_ROLE_SEQ, this.SECRETARY_ROLE_SEQ];
  notestepname: string = '';
  ...
  vStartDate: string = '';
  vEndDate: string = '';
  private uploadCertificateFile: string = '';
  selectedIndex: number = 0;
  private editnotemodalRef: any;

  updateNote: string = '';
  hasacceptedquery: boolean = false;
  nextstepseq: number=0;

  constructor(private rest: RestApiService, private notifier: NotifierService, private roleApiService: MstWorkflowStepsRoleUserService, private route: Router, private projectlistService: ProjectListService,
  ...
  ngOnInit(): void {
    ...
    this.agentrenewalid = this.common.getRenewalId();
    this.workflowtype = this.common.getWorkflowType();
    // console.log(this.agentId);
    this.steps = [];
    if (this.workflowtype === this.AGENT_TYPE || this.workflowtype === this.AGENT_RENEWAL_TYPE) {
      this.isAgent = true;
    }
  ...
  fetchProjectDetailsById(): void {
    ...
    if (this.isAgent) {
      ...
      if (this.workflowtype === this.AGENT_RENEWAL_TYPE) {
        data['agentrenewalid'] = this.agentrenewalid;
      }
    } else {
    ...
    this.apiService.fetchProjectDetailsById(data).subscribe((res: any) => {
      ...
      if (res.success) {
        if (res.response.length > 0) {
          console.log(res.response[0].isoldqprapplicable);
          if (res.response[0].isoldqprapplicable) {
            this.isOldProject = res.response[0].isoldqprapplicable === 1;
          }
          ...
          this.isCertificate = (res.response[0].certificate !== '');
          ...
          if (this.displaynotequerysteps.indexOf(this.roletype) > -1) {
          ...
          if (this.displaynotesteps.indexOf(this.roletype) > -1) {
            this.isdisplaynotetab = true;
          }
          if (this.displayceosteps.indexOf(this.roletype) > -1) {
  ...
  getDocProjectWise() {
    ...
    this.apiService.getDocProjectWise(data).subscribe((res: any) => {
      if (res.success) {
      ...
      this.apiService.fetchChecklistByTab(data).subscribe((res: any) => {
        ...
        if (res.success) {
          ...
          for (let i = 0; i < this.tabdetails.length; i++) {
            for (let j = 0; j < this.tabdetails[i].details.length; j++) {
              const showIcon = (/\.(gif|jpg|jpeg|tiff|png|pdf)$/i).test(this.tabdetails[i].details[j].projectfieldvalue)
              console.log("Res"+this.tabdetails[i].details[j].checklistname+"=="+this.tabdetails[i].details[j].projectfieldvalue)

              this.tabdetails[i].details[j].showIcon = showIcon;
              if (this.tabdetails[i].details[j].details) {
                for (let k = 0; k < this.tabdetails[i].details[j].details.length; k++) {
                  const showIcon = (/\.(gif|jpg|jpeg|tiff|png|pdf)$/i).test(this.tabdetails[i].details[j].details[k].projectfieldvalue)
                  // console.log("Response2>>>>>>>>"+this.tabdetails[i].details[j].details[k].projectfieldvalue)
  ...
  onTabChange(event: any): void {
    ...
    if (this.tabs.length > 0) {
      if (this.workflowtype === this.REGISTRATION_TYPE) {
        ...
        if (this.selectedTabIndex > 4 && this.selectedTabIndex < 5 + this.tabs.length) {
          this.fetchChecklistByTab(this.tabs[this.selectedTabIndex - 5].id);
        }
      } else {
    ...
    if (event.tab.textLabel === 'Submit') {
      ...
      this.getSingleQueryAnswer(this.extensionId).then((resp: any) => {
        ...
        for (let i = 0; i < this.queryanswerdetails.length; i++) {
          if (this.queryanswerdetails[i].isvalidquery === 1) {
            this.hasacceptedquery = true;
            break;
          }
        }
      });
        ...
        this.getApprovalNotes().then(() => {
          if (this.stepseqno !== this.HOR_STEP_SEQ && this.stepseqno !== this.CEO_STEP_SEQ) {
            ...
            if (this.isCertificate) {
              ...
              } else if (this.workflowtype === this.AGENT_RENEWAL_TYPE) {
                this.getAgentRenewalCertificate('edit', val, comment);
              }
            }
          }
        });
      ...
      if (this.workflowtype === this.EXTENSION_TYPE) {
        ...
        this.regplaceholder = 'Enter Agent Renewal No.';
      }
      this.getApprovalNotes().then(() => {
        let comment = ''
        for (let i = 0; i < this.approvalnotes.length; i++) {
  ...
  insertinternalnotes(type: number) {
    ...
    this.apiService.insertInternalNotes(data).subscribe((res: any) => {
      ...
      if (res.success) {
        ...
        this.notes.push({notes: this.internalNote, username: this.common.getUsername(),userid:this.userID});
        this.internalNote = '';
      } else {
        this.notifier.notify('error', res.message);
      }
  ...
  moveWorkflow(id: any, roleid = 0, userid = 0) {
    ...
    // alert(id);
    if(this.nextstepseq == this.PROMOTER_STEP_SEQ && !this.hasacceptedquery){
      this.notifier.notify('error', "No accepted query to send.Please accept atleast one query to send.");
    }else {
      let data: any = {};
      data = {
        reraid: this.common.getReraId(),
  ...
  uploadFile(e: any, i: any, type: any, j = 0) {
    ...
    xhr.setRequestHeader("Authorization", `Bearer ${this.common.getToken()}`);
    xhr.send(formData);
  }
  ...
  uploadCertificate(e: any) {
    ...
    formData.append("file", file);
    this.common.loaderStart();
    this.rest.certificateUpload(formData).subscribe((res: any) => {
      this.common.loaderEnd();
      if (res.success) {
        this.uploadCertificateFile = res.response.fileName;
        console.log(this.uploadCertificateFile)
      }
    }, (err: any) => {
      this.common.loaderEnd();
    })
    ...
    //   console.log(xhr.responseText)
    //   if (xhr.status === 200 && xhr.responseText !== '') {
    //     this.uploadCertificateFile=JSON.parse(xhr.responseText).fileName;
    //     console.log(this.uploadCertificateFile)
    //
    //   }
    // };
    ...
    // xhr.onreadystatechange = state => {
    //   try {
    //     let resp = this.common.decryptPayload(xhr.responseText.substring(1, xhr.responseText.length - 1))
    //     console.log(JSON.stringify(resp))
    //     if (resp.status === 200) {
    //       this.uploadCertificateFile=resp.fileName;
    //       console.log(this.uploadCertificateFile)
    //     }
    //   }catch(e){
    //
    ...
    // };
    // xhr.timeout = 5000;
    // xhr.open("POST", this.projectlistService.API_ROOT + this.projectlistService.PROJECT_PATH + 'certificateUpload');
    // xhr.setRequestHeader("Authorization", `Bearer ${this.common.getToken()}`);
    // xhr.send(formData);
  }
      ...
      if (!isRegBlank) {
        if (this.workflowtype === this.EXTENSION_TYPE) {
          ...
          const data = {
            ...
            status: approved,
            regno: this.regno,
            agentrenewalid: this.agentrenewalid
          };
          this.common.loaderStart();
          this.apiService.updateAgentRenewalStatus(data).subscribe((res: any) => {
            this.common.loaderEnd();
            if (res.success) {
              this.notifier.notify('success', res.message);
              sessionStorage.removeItem("pid");
  ...
  getAgentRenewalCertificate(type: string, fieldtype: string, reason: string) {
    const data = {
      reraid: this.common.getReraId(),
      agentid: this.agentId,
      userid: this.common.getUserId(),
    ...
    // console.log(JSON.stringify(data));
    this.common.loaderStart();
    this.apiService.getAgentRenewalCertificate(data).subscribe((res: any) => {
      this.common.loaderEnd();
      if (res.success) {
        this.certificate = res.response.fileName;
        this.isCertificate = true;
  ...
  onStateChange() {
    ...
    for(let i=0;i<this.nextSteps.length;i++){
      if(this.nextSteps[i].id==this.nextstepid){
        this.nextstepseq=this.nextSteps[i].seqno;
      }
    }
    // console.log(this.nextstepseq)
        ...
        if (res.success) {
          ...
          if (this.roletype === this.CHAIRMAN_ROLE_SEQ) {
            ...
            if (this.workflowtype === this.EXTENSION_TYPE) {
              ...
              this.getAgentRenewalCertificate('open', val, this.approvalNote);
            }
          }
        } else {
          this.notifier.notify('error', res.message);
  ...
  updateCertificate() {
    const collection = document.getElementsByClassName("angular-editor-textarea");
    let html = collection[0].innerHTML.replace(/\n/g, '');
    // html=html.replace(/\"/g,'"');
    // console.log(html);
    ...
    if (this.workflowtype === this.REGISTRATION_TYPE) {
      ...
      this.common.loaderStart();
      this.apiService.saveAgentRenewalCertificateContent(data).subscribe((res: any) => {
        this.common.loaderEnd();
        if (res.success) {
          this.notifier.notify('success', 'Certificate changed successfully');
          this.certificate = res.response.filename;
  ...
  ViewDocument(file: string) {
    ...
    window.open(this.projectlistService.BASE_ROOT + 'uploadedFiles/' + file, '_blank');
  ...
  uploadCertDetails() {
    if (this.workflowtype === this.REGISTRATION_TYPE) {
      let data = {
        certificatefile: this.uploadCertificateFile,
        validitystartdate: this.vStartDate,
        validityenddate: this.vEndDate,
        projectid: this.projectId
      };
      // console.log(JSON.stringify(data))
      if (!this.common.isBlankField(data)) {
        this.common.loaderStart();
        this.apiService.updateRegCertificate(data).subscribe((res: any) => {
          this.common.loaderEnd();
          if (res.success) {
            this.notifier.notify('success', 'Certificate Updated Successfully');
          } else {
            // alert(res.message);
            this.notifier.notify('error', res.message);
          }
        ...
        this.notifier.notify('error', 'Please add all the fields');
      }
    } else if (this.workflowtype === this.EXTENSION_TYPE) {
      let data = {
        certificatefile: this.uploadCertificateFile,
      ...
      if (!this.common.isBlankField(data)) {
        this.common.loaderStart();
        this.apiService.updateExtensionCertificate(data).subscribe((res: any) => {
          this.common.loaderEnd();
          if (res.success) {
            this.notifier.notify('success', 'Certificate Updated Successfully');
          } else {
      ...
      if (!this.common.isBlankField(data)) {
        this.common.loaderStart();
        this.apiService.updateAgentCertificate(data).subscribe((res: any) => {
          this.common.loaderEnd();
          if (res.success) {
            this.notifier.notify('success', 'Certificate Updated Successfully');
          } else {
      ...
      if (!this.common.isBlankField(data)) {
        this.common.loaderStart();
        this.apiService.updateRenewalCertificate(data).subscribe((res: any) => {
          this.common.loaderEnd();
          if (res.success) {
            this.notifier.notify('success', 'Certificate Updated Successfully');
          } else {
  ...
  openEditNotes(i: any) {
    ...
    this.updateNote = this.notes[i].notes;
    // console.log(this.updateNote);
    this.editnotemodalRef = this.modalService.open(this.editNote, {size: ''});
    // this.editnotemodalRef.result.then((result:any) => {
    // }, (reason:any) => {
    //
    // });
  }
  ...
  updateNotes() {
    let data = {
      id: this.notes[this.selectedIndex].id,
      notes: this.updateNote,
    };
    // console.log(JSON.stringify(data))
    this.common.loaderStart();
    this.apiService.updateInternalNotes(data).subscribe((res: any) => {
      this.common.loaderEnd();
      if (res.success) {
        this.notifier.notify('success', 'Notes Updated Successfully');
        this.notes[this.selectedIndex].notes = this.updateNote;
        this.closeModal();
      } else {
        // alert(res.message);
        this.notifier.notify('error', res.message);
  ...
  onFocusOutEvent($event: any,val:any) {
    console.log('focus out');
    console.log($event.target.value);
    console.log(JSON.stringify(val));
    val.mstworkflowstepsroleuserchecklistid = val.id;
    delete val.id;
    let data: any = {}
    data = {
      reraid: this.common.getReraId(),
